...
public class Solver {
     ...
     * 不变 | 需要迭代的队列，贪婪算法
     */
    static MinPQ<Node> minPQ = new MinPQ<>();
    /**
     * 不变 | 成功搜索后，得到的正序路径
     */
    ...
    public Solver(Scanner sc) {
        ...
        boolean flag = GreedySearch();

        if (flag) {
            new MyGUI(600, 600, boards, m, n);
        }
    }
    ...
    public boolean GreedySearch() {
        ...
        boolean flag = false;
        try {
            while (!minPQ.isEmpty() && !flag) {
                //若minPQ过大，进行剪枝 | 玄学
                if (minPQ.size() > 500000) {
                    MinPQ<Node> tempPQ = new MinPQ<>();
                    for (int i = 0; i < 10000; i++) {
                        tempPQ.insert(minPQ.delMin());
                    }
                    minPQ = tempPQ;
                }
                flag = search(minPQ.delMin());
            }
    ...
    /**
     * 进行搜索，并自动将valid子节点存储到nodeSet和minPQ中，如果找到答案，则给出相应flag
     * valid条件：子节点不能是nodeSet中已经存在的点
     *
     * @return true：search的子节点中出现答案，并将最后的节点存储到node中 | false：search的子节点中无答案，在bfs中继续迭代
     */
